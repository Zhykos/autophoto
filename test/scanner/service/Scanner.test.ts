...
Deno.test(async function scanAndSaveNewImages() {
  await beforeEach();
  ...
  try {
    ...
    assertEquals(new Set(filesAfterScan.map((f) => f.uuid)).size, 8);
    ...
    const linksVideoGameIds = [...new Set(allLinks.map((l) => l.videoGameID))];
